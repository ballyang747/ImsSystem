 ...
 function close(){
     ...
     $(function(){
		...
		$.ajax({
			...
			success:function(obj){
				...
				$.each(depts,function(){
					...
					var jobs = obj.jobs;
				$.each(jobs,function(){
					$("<option>").val(this.code).text(this.name).prop("selected",this.id=="${user.job.code}").appendTo("#jobSelect");
				})
			},error:function(obj){
				 alert("网络异常！");
			}
		...
		$.ajax({
			type:"post",
			dataType:"json",
			url:"${ctx}/identity/user/ajaxLoadModules.jspx?pCode=${pCode}",
			success:function(obj){
				alert(obj);
			},error:function(obj){
				 alert("网络异常！");
		      	   ...
		      	   if(r){
		  			 //定义数组用于存储需要删除的用户的id
		  			   ...
		  			   $.each(boxes,function(){
		  				   ...
		  				   window.location="${ctx}/identity/user/deleteUser.jspx?userIds="+array.join(",")+
		  				   "&pageIndex=${pageModel.pageIndex}&name=${user.name}&phone=${user.phone}&dept.id=${user.dept.id}&job.code=${user.job.code}";
		  			   })
		  			}	 
			 });
     ...
     function preUser(userId){
 		...
 		$("#iframe").prop("src","${ctx}/identity/user/showpreUser.jspx?userId="+userId)
...
<!-- 						</td> -->
						...
						<fk:Operas value="/identity/user/addUser.jspx"><a  id="addUser" class="btn btn-success"><span class="glyphicon glyphicon-plus"></span>&nbsp;添加</a></fk:Operas>
					    <fk:Operas value="/admin/identity/deleteUser.jspx"><a  id="deleteUser" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span>&nbsp;删除</a></fk:Operas>
					 </td>
					</tr>
					</tbody>
				</table>
 		...
 		<div class="panel panel-primary" style="padding-left: 10px;">
			...
			<div class="panel-body" >
			    ...
			    <fk:Pager pageIndex="${pageModel.pageIndex}" pageSize="${pageModel.pageSize}" totalNum="${pageModel.recordCount}" submitUrl="${ctx}/identity/user/selectUser.jspx?pageIndex={0}&name=${user.name}&phone=${user.phone}&dept.id=${user.dept.id}&job.code=${user.job.code}" pageStyle="flickr"></fk:Pager>

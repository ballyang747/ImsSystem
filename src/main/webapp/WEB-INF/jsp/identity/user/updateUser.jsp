	...
	$(function() {
		...
		$.ajax({
			...
			success:function(obj){
				var depts = obj.depts;
				$.each(depts,function(){
					$("<option>").val(this.id).text(this.name).attr("selected",this.id=="${user.dept.id}").appendTo("#deptSelect");
				})
					var jobs = obj.jobs;
				$.each(jobs,function(){
					$("<option>").val(this.code).text(this.name).attr("selected",this.id=="${user.job.code}").appendTo("#jobSelect");
				})
			},error:function(obj){
				 alert("网络异常！");
			}
		...
		$("#btn_submit").click(function() {
			...
			if(msg){
				...
				$.post("${ctx}/identity/user/updateUser.jspx",
       				 ...
       				 function(msg1,status){
       			         ...
       			         if(status=="success"&& msg1=="ok"){
       			        	  ...
       			        	  sessionStorage.setItem("msg","用户修改成功");
       			        	 parent.close();
                             }else if(status=="success"&& msg1=="fail"){
                           	  sessionStorage.setItem("msg","用户修改失败");
        			        	 parent.close();
                             }else{

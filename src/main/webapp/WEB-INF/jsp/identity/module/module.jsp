				 ...
				 $.messager.confirm('提示信息', "是否确认删除？", function(r){
			      	   if(r){
			  			 //定义数组用于存储需要删除的用户的id
			  			   ...
			  			   window.location="${ctx}/identity/module/deleteModule.jspx?codes="+array.join(",")+
		  				   "&pageIndex=${pageModel.pageIndex}&pCode=${pCode}";
			  			}	 
				 });
			}	 
		 });	
...
<%-- 							<td>${module.remark}</td> --%>
							<td>${module.url}</td>
							<td><span class="label label-success"><a href="${ctx}/identity/module/getModulesByParent.jspx?pCode=${module.code}" style="color: white;">查看下级</a></span></td>
...
<%-- 							<td><fmt:formatDate value="${module.modifyDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
				...
				<!-- 分页标签区 -->
				 <fk:Pager pageIndex="${pageModel.pageIndex}" pageSize="${pageModel.pageSize}" totalNum="${pageModel.recordCount}" submitUrl="${ctx}/identity/module/getModulesByParent.jspx?pageIndex={0}&pCode=${pCode}" pageStyle="flickr"></fk:Pager>
